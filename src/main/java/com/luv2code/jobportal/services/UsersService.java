...
public class UsersService {
    ...
    public Users addNew(Users users) {
        ...
        UsersType type = usersTypeRepository.findById(
        ...
        if (type.getUserTypeName().equalsIgnoreCase("Recruiter")) {
            recruiterProfileRepository.save(new RecruiterProfile(saved));
        } else {
            jobSeekerProfileRepository.save(new JobSeekerProfile(saved));
        }
    ...
    public Object getCurrentUserProfile() {
        ...
        if (!(auth instanceof AnonymousAuthenticationToken)) {
            ...
            if (auth.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_RECRUITER"))) {
                return recruiterProfileRepository.findById(user.getUserId()).orElse(null);
            }
            return jobSeekerProfileRepository.findById(user.getUserId()).orElse(null);
        }
        return null;
    }
